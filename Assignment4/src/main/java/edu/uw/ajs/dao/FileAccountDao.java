	...
	public Account getAccount(final String accountName) {
		...
		if (accountDir.exists() && accountDir.isDirectory()) {
			try {
				...
				logger.warn(String.format("Unable to access or read account data, '%s'", accountName), ex);
			} catch (final AccountException ex) {
				logger.warn(String.format("Unable to process account files for account, '%s'", accountName), ex);
			} finally {
				if (in != null) {
					try {
						in.close();
