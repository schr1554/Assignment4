...
public class SimpleBroker implements Broker, ExchangeListener {
	...
	public SimpleBroker(final String brokerName, final AccountManager acctMgr, final StockExchange exchg) {
		...
		marketOrders = new SimpleOrderQueue<>("Thread " + threadNameCount, exchg.isOpen(), (t, o) -> t);
	 ...
	 */
	public synchronized final Account createAccount(final String username, final String password, final int balance)
	 ...
	 *
	 * @exception BrokerException
	 *                if unable to obtain the order manager
	 */
	 ...
	 */
	public synchronized final StockQuote requestQuote(final String symbol) throws BrokerException {
		checkInvariants();
	...
	public synchronized void close() throws BrokerException {
		try {
			...
			throw new BrokerException("Attempt to close the broker failed.", ex);
		}
	}

	/**
	 * Every public operation invokes this method to insure the broker is
	 * capable of performing operations. If the broker is not in a valid state
	 * an IllegalStateException is thrown.
	 * 
	 * @throws IllegalStateException
	 *             if broker is n an invalid state
	 */
	private void checkInvariants() {
		if (name == null || accountManager == null || stockExchange == null || orderManagerMap == null
				|| marketOrders == null) {
